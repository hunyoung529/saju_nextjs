...
function Matchlist() {
  ...
  const currentUserGender =
    typeof window !== "undefined" ? sessionStorage.getItem("gender") : null;
  ...
  const isLiked = (member) =>
    likeMember.some((item) => item.opntid === member.id);
  ...
  const matchingData = () => {
    axios
      .get(`/api/fortune?id=${currentUserID}&type=fortuneCheck`)
      .then((res) => {
        ...
        Fdata.forEach((obj) => {
          ...
          try {
            for (let k = 0; k < myData.length; k++) {
              ...
              if (matchCount >= 2) {
                let genderCheck = genderFilteredMdata.filter(
                  (g) => g.id == obj.id
                );
          ...
          {matchedMembers.map(({ member, matchingFdata }) => (
            <SwiperSlide key={member.id} className={matchlist.swiper_slide}>
              <div className={matchlist.slide_content}>
                ...
                <p>{member.address} 거주</p>
                <p>{member.job}</p>
                <p>{member.self}</p>

                {matchingFdata && (
                  <p className={matchlist.match_fortune}>
                    {matchingFdata.myelement}
                  </p>
                )}
              </div>
              ...
              <div className={matchlist.match_button}>
                <button
                  ref={btnRef}
                  className={`${matchlist.postbtn} ${
                    actItem === btnRef.current ? matchlist.on : ""
                  }`}
                  onClick={(e) => {
                    post(e, member);
                    classAct(btnRef);
