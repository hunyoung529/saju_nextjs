...
export async function POST(req) {
  ...
  const data = await queryExecute(
    `insert into matchlist(id, opntid, m_status, y_status, date) values(?,?,?,?,?)`,
    [id, opntid, m_status, y_status, date]
  );
...
export async function GET(req) {
  ...
  let data;
  if (myPick === "true") {
    data = await queryExecute(`SELECT * FROM b3o2.matchlist WHERE id='${id}'`);
  } else {
